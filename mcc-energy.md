...
1. 终端系统上可offloading部分的实际非常有限，当前工作只考虑应用范围代码迁移实现的计算迁移和云端融合。而实际上应用运行时能耗高的代码部分是应用之外的终端平台底层框架调用能耗，虽然框架代码被应用调用是完成的应用逻辑功能，但由于框架本身被调用执行的计算并不在当前工作的考虑范畴内，所以当前研究工作并不以其为优化目标。因此如何实现全平台的迁移是提升云端融合技术有效性的关键之一；

2. offloading中云端终端的异构性。当前云端融合技术主要在托管代码环境中实现计算迁移，这一方式在达成实现简单的目标的同时，限制了云端融合技术的应用范围。从云计算技术“按需使用”的基本理念角度来说，云端资源不仅应该时间空间上的按需可用，更重要的是对资源形式的灵活可定制，当终端需迁移的计算形态与云端不兼容时，云端资源如何通过虚拟化等技术进行有效适配从而云端融合适用场景，是我们在云计算技术方面还需要的研究；

2. offloading 的决策很困难，因为能耗模型并不精确（能耗长尾效应）、并且受多种因素影响（包括网络状况、代码能耗预测等），因此如何做出精准决策也是云端融合技术中的难点；offloading也很难做到全局优化，应用本身是多线程并发运行的，终端系统中也会同时存在多个运行中的应用，如何为这些应用及其执行线程进行offloading的调度需要进行深入权衡，调度策略既考虑单个应用的响应时间等性能指标，又需要从全局角度进行offloading请求合并或延后以优化全局能耗。
